...
Images of X-ray CT scans of a stationary 3D printed sample were obtained for this project. The aim of this specific project was to find a mean and variance relationship of the grey values in the pixels. This was done by fitting linear regressions. Separately, latent variable models were fitted to the images to find sources of variance. Latent variable models included principle component analysis, factor analysis and compound Poisson.
...
\end{itemize}
From this, some additional properties of the X-ray was estimated. It is given that the efficiency of photon production $\eta$ is \cite{michael2001x}
\begin{equation}
\eta = aVZ =8.1\times10^{-3}
\end{equation}
where $a=1.1\times 10^{-1}\text{ V}^{-1}$ is a constant and $Z=74$ is the atomic number of tungsten. The rate of photon production $\nu$ and energy per photon $E$ can be calculated
\begin{equation}
...
\subsection{Theory}
The sample variance is a random variable, thus itself has uncertainty. Weight-ed least squares was used to take into consideration the uncertainty of the sample variance. This was done by giving low weights to data points with high uncertainty on the grey value variance.
...
\end{equation}
for each polynomial order and repeated for 40 different bootstrap samples. A bootstrap sample was obtained by randomly resampling $m$ times without replacement from the original dataset \cite{efron1992bootstrap}. 
...
\begin{figure}
	...
	\begin{subfigure}{0.45\textwidth}
		...
		\caption{Order 6}
	\end{subfigure}
	\caption{Frequency density of the sample mean-variance of the grey values. Weighted least squares with different orders of polynomial features were fitted. The (red) dotted line represent the 95\% prediction interval.}
...
\begin{figure}
	...
	\caption{BIC, for 40 different bootstrap samples, for fitting weighted least squares with polynomial features on the sample mean-variance pairs.}
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Frequency density of the sample mean-variance of the resampled grey values for each material. Weighted linear regression with polynomial features was fitted on a subsampled of sample mean-variance pairs for each material. The (red) dotted line represent the 95\% prediction interval.}
...
In this project, only a mixture of 1st order linear regression was considered. The EM algorithm was initialised by assigning random responsibilities to all models and data points in the E step. The number of mixture was set to $q=3$ to see if this model captured the 3 materials. 50 EM steps were used.
...
\subsection{Results}
Figure \ref{fig:initial_PCvariance} shows the first 6 of the diagonal of the outer product of the individual principle components, they represent the normalised main sources of variance. It was observed that on the first and second principle components, there are sources of variance on the bottom right and top right of the image. This could imply that the sources of variance in the background were not uniformly distributed in space.
...
\end{equation}
As a result there are two sources of variance, factor noise from the $\matr{\Lambda}\matr{\Lambda}\T$ component and the intrinsic noise $\matr{\Psi}$. Such a model is called factor analysis \cite[pp.~462-469]{barber2012bayesian}. The model can be extended for non-zero mean easily by adding an arbitrary constant. $\vect{X}$ was used to represent the grey values of all pixels in an image.
...
\end{align}
The loading matrix $\matr{\Lambda}$ and intrinsic noise $\matr{\Psi}$ can be estimated by maximising the log likelihood. This is best done using the EM algorithm \cite[pp.~260-261]{barber2012bayesian}.
...
The BIC was used to select the best $k$ to use. The BIC is given as
\begin{equation}
\BIC=-2\ln L + p(k+1)\ln n \ .
\end{equation}
The data was bootstrapped to assess the uncertainty in the BIC.
...
The 8 factor variances were isolated and as shown in Figure \ref{fig:initial_factor_factorNoise}, together with the intrinsic variance as shown in Figure \ref{fig:initial_factor_instrinicNoise}. It was interesting to note that in Figure \ref{fig:initial_factor_factorNoise}, there are clusters of high variance in most of the factors. This suggested clusters of high variance can appear in the data, thus the variance of each of the pixel's gray values can depend on each other spatially. In Figure \ref{fig:initial_factor_instrinicNoise}, the shape of the 3d printed sample can be seen represented by the low intrinsic variance. This was evidence that there may well be a mean and variance relationship because the 3D printed sample has lower sample mean grey values.
...
\begin{figure}
	...
	\caption{The 8 individual factor variances with the highest log likelihood found.}
...
\right]
\end{equation}
where $\left(x_i^{(j)}\right)^2\zeta_i^{(j)}=0$ if $\left(x_i^{(j)}\right)^2=0$. This is derived in Appendix \ref{chapter:mStep_compoundPoisson}. It is also given that the Cram\'er–Rao lower bounds are
\begin{equation}
...
\right]
\end{equation*}
Because the marginal distribution of the latent variables does not depend on $\matr{\Lambda}$ and $\matr{\Psi}$, the last term can be ignored. Using Equation \eqref{eq:factor_analysis_x_given_y_dist}
\begin{multline*}
...
\begin{multline*}
H = -\frac{1}{2}
\sum_{i=1}^{n}\expectation\left[\left(\ln{\|2\pi\matr{\Psi}\|}+\left(\vect{x}^{(i)}\right)\T\matr{\Psi}^{-1}\matr{\Lambda}\vect{Y}^{(i)}
\right.\right.
\\
\left.\left.
...
+c_2 \ .
\end{multline*}
Taking the derivative with respect to $\matr{\Psi}$, using the results from Appendix \ref{chapter:matrixCalculus},
\begin{multline*}
...
\section{Cram\'er-Rao Lower Bounds for Compound Poisson}\label{chapter:crBound_compoundPoisson}
Suppose the latent variables are known. The joint log likelihood as a function of random variables is
\begin{multline}
\ln{L}= -n\nu_i\tau+\ln(\nu_i\tau)\sum_{j=1}^nY_i^{(j)}-\frac{n}{2}\ln\left(\frac{\alpha^2\sigma_i^2}{\tau^2}\right)
\\
...
\end{pmatrix}\right] \ .
\end{equation}
By substituting in the joint log likelihood, it can be shown that
\begin{multline}
\matr{I}=\expectation\left[
\left(\begin{matrix}
