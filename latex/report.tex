...
The expected energy can be calculated by splitting the rate density function into $n$ equal intervals, of size $\Delta E$, $E_1,E_2,\dotdotdot,E_n$. For each interval, a weighted mean can be used to estimate the expected energy
\begin{equation}
...
\begin{equation}
I_1=\frac{1}{A}\sum_{i=1}^n\euler^{-\mu(E_i)L}E_i\lambda_0(E_i)\Delta E_i \ .
\end{equation}
Taking the limit $\Delta E\rightarrow0$ and $n\rightarrow\infty$, the sum becomes an integral and can be generalised to
\begin{equation}
...
\section{Intensity}
Let $U_i$ be the energy of photon $i$. Let $T$ be the sum of all the $N$ photon's energies detected after some exposure time $t$. These can be modelled as
\begin{equation}
