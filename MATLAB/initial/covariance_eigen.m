...
%declare array of eigenvalues and PC1
eigenvalue_array_core = zeros(n_bootstrap,6,n_core);
PC1_array_core = zeros(area,n_bootstrap,n_core);
eigenvalue_array = zeros(n_bootstrap*n_core+1,6);
PC1_array = zeros(area,n_bootstrap*n_core+1);
...
S = eigenvector;
%save the non-bootstrap eigenvalue at the end
...
%reshape the matrix so that each column correspond to an eigenvalue
for i = 1:n_core
...
%for each eigenvector, plot the eigenimage
figure;
for i = 1:6
    subplot(2,3,i);
...
%save the variables
save('eigenspectrum.mat','R','eigenvalue_array','S','PC1_array','n_core','n_bootstrap');